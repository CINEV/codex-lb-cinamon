...
async def test_core_inline_input_image_urls_converts_top_level_input_image(monkeypatch):
    ...
    async def fake_fetch_image_data_url(_session, image_url, _timeout, *, deadline=None):
        del deadline
...
async def test_stream_responses_starts_upstream_timer_after_image_inlining(monkeypatch):
    ...
    async def fake_inline(payload_dict, session, connect_timeout, *, deadline=None):
        del session, connect_timeout, deadline
        nonlocal inline_ran
        inline_ran = True
        return payload_dict
...
async def test_inline_http_bridge_image_urls_converts_external_urls(monkeypatch):
    ...
    async def fake_inline(payload_dict, _session, _timeout, *, deadline=None):
        del deadline
...
async def test_inline_http_bridge_image_urls_converts_top_level_input_image(monkeypatch):
    ...
    async def fake_inline_nested(payload_dict, _session, _timeout, *, deadline=None):
        del deadline
        return dict(payload_dict)

    async def fake_inline_content(content, _session, _timeout, *, deadline=None):
        del deadline
...
async def test_inline_http_bridge_image_urls_rejects_when_fetch_fails(monkeypatch):
    ...
    async def fake_inline_noop(payload_dict, _session, _timeout, *, deadline=None):
        del deadline
