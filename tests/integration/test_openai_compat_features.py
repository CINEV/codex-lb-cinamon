...
async def test_backend_responses_allows_builtin_tools(async_client, monkeypatch, tool_payload):
    ...
    assert resp.status_code == 200
    expected_tools = [] if tool_payload["type"] == "image_generation" else [tool_payload]
    assert seen["payload"].tools == expected_tools
...
async def test_v1_chat_completions_reserves_enforced_service_tier(async_client, monkeypatch):
    ...
    async def fake_enforce(api_key, *, request_model, request_service_tier, request_usage_budget=None):
        del request_usage_budget
