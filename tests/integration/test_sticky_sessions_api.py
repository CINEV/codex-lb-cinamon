        ...
        for i in range(bulk_count):
            await session.execute(
                text(
                    """
                    INSERT INTO sticky_sessions (
                        key, account_id, kind, provider_kind, routing_subject_id, created_at, updated_at
                    )
                    VALUES (
                        :key, :account_id, :kind, :provider_kind, :routing_subject_id, :timestamp, :timestamp
                    )
                    """
                ),
                {
                    ...
                    "routing_subject_id": accounts[0].id,
