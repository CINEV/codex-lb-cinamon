...
def _service_core_transcribe_audio() -> Callable[..., Awaitable[dict[str, JsonValue]]]:
    provider_module = sys.modules.get("app.modules.proxy.provider_adapters")
    if provider_module is not None:
        provider_default = getattr(provider_module, "_DEFAULT_CORE_TRANSCRIBE_AUDIO", None)
        provider_candidate = getattr(provider_module, "core_transcribe_audio", provider_default)
        if provider_candidate is not None and provider_candidate is not provider_default:
            return cast(Callable[..., Awaitable[dict[str, JsonValue]]], provider_candidate)

    service_module = sys.modules.get("app.modules.proxy.service")
    if service_module is not None:
        service_candidate = getattr(service_module, "core_transcribe_audio", core_transcribe_audio)
        if provider_module is not None:
            provider_default = getattr(provider_module, "_DEFAULT_CORE_TRANSCRIBE_AUDIO", None)
            if service_candidate is not provider_default:
                return cast(Callable[..., Awaitable[dict[str, JsonValue]]], service_candidate)
        return cast(
