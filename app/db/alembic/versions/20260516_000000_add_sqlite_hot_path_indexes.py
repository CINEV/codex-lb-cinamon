...
def upgrade() -> None:
    ...
    if (
        reservation_indexes is not None
        and "idx_api_key_usage_reservations_status_updated_at" not in reservation_indexes
    ):
        op.create_index(
    ...
    if bridge_indexes is not None and "idx_http_bridge_sessions_latest_turn_scope_state_seen" not in bridge_indexes:
        op.create_index(
    ...
    if bridge_indexes is not None and "idx_http_bridge_sessions_latest_response_scope_state_seen" not in bridge_indexes:
        op.create_index(
...
def downgrade() -> None:
    bridge_indexes = _index_names("http_bridge_sessions")
    if bridge_indexes is not None and "idx_http_bridge_sessions_latest_response_scope_state_seen" in bridge_indexes:
        op.drop_index("idx_http_bridge_sessions_latest_response_scope_state_seen", table_name="http_bridge_sessions")
    if bridge_indexes is not None and "idx_http_bridge_sessions_latest_turn_scope_state_seen" in bridge_indexes:
        op.drop_index("idx_http_bridge_sessions_latest_turn_scope_state_seen", table_name="http_bridge_sessions")

    reservation_indexes = _index_names("api_key_usage_reservations")
    if reservation_indexes is not None and "idx_api_key_usage_reservations_status_updated_at" in reservation_indexes:
