...
type MockState = {
  ...
  stickySessions: Array<{
    ...
    providerKind: "chatgpt_web" | "openai_platform";
...
export const handlers = [
  ...
  http.post("/api/accounts/platform", async ({ request }) => {
      ...
      return HttpResponse.json(
        {
          error: {
            ...
            message: "Invalid platform identity payload",
          },
        },
        { status: 422 },
      );
    }
    const hasActiveChatGptAccount = state.accounts.some(
      (account) =>
        account.providerKind !== "openai_platform" &&
        account.status === "active",
    );
    if (!hasActiveChatGptAccount) {
      return HttpResponse.json(
        {
          error: {
    ...
    if (
      state.accounts.some(
        (account) => account.providerKind === "openai_platform",
      )
    ) {
      return HttpResponse.json(
        {
    ...
    const sequence = state.accounts.length + 1;
    const accountId = `platform_${sequence}`;
    const created = createAccountSummary({
      accountId,
      email: payload.label,
  ...
  http.patch("/api/accounts/platform/:accountId", async ({ params, request }) => {
    ...
    if (payload.organization !== undefined) {
      account.organization = payload.organization;
    }
    if (payload.project !== undefined) {
      account.project = payload.project;
    }
    ...
    const filteredEntries = state.stickySessions
      .filter((entry) => {
        ...
        if (
          ...
          ].some((value) => value.toLowerCase().includes(accountQuery))
        ) {
          return false;
        }
  ...
  http.delete("/api/sticky-sessions/:kind/:key", ({ params, request }) => {
    ...
    const providerKind =
      url.searchParams.get("providerKind") === "openai_platform"
        ? "openai_platform"
        : "chatgpt_web";
    ...
    if (state.stickySessions.length === before) {
      return HttpResponse.json(
  ...
  http.post("/api/sticky-sessions/delete", async ({ request }) => {
        ...
        sessions: z
          .array(
            z.object({
              ...
              providerKind: z
                .enum(["chatgpt_web", "openai_platform"])
                .default("chatgpt_web"),
            }),
          )
          .min(1)
          ...
          .refine(
            (sessions) =>
              new Set(
                sessions.map(
                  (session) =>
                    `${session.providerKind}:${session.kind}:${session.key}`,
                ),
    ...
    const targets = new Set(
      payload.sessions.map(
        (session) => `${session.providerKind}:${session.kind}:${session.key}`,
      ),
    );
    const deleted = state.stickySessions
      .filter((entry) =>
        targets.has(`${entry.providerKind}:${entry.kind}:${entry.key}`),
      )
      .map((entry) => ({
        key: entry.key,
        kind: entry.kind,
    ...
    const deletedTargets = new Set(
      deleted.map(
        (entry) => `${entry.providerKind}:${entry.kind}:${entry.key}`,
      ),
    );
    state.stickySessions = state.stickySessions.filter(
      (entry) =>
  ...
  http.post("/api/sticky-sessions/delete-filtered", async ({ request }) => {
    ...
    const providerKind = payload.providerKind ?? null;
    const matched = state.stickySessions.filter((entry) => {
      ...
      if (providerKind && entry.providerKind !== providerKind) {
        return false;
      }
      if (
        accountQuery &&
