...
describe("RecentRequestsTable", () => {
  ...
  it("renders rows with status badges and supports request details and copy actions", async () => {
    ...
    expect(within(dialog).getAllByText("upstream-req-1")[0]).toBeInTheDocument();
    expect(within(dialog).getByText("platform-subject-1")).toBeInTheDocument();
    expect(within(dialog).getAllByText("priority")[0]).toBeInTheDocument();
    expect(within(dialog).getAllByText("default")[0]).toBeInTheDocument();
    expect(within(dialog).getAllByText("provider_transport_unsupported")[0]).toBeInTheDocument();
