...
export function ModelMultiSelect({
  ...
  const label =
    value.length === 0 ? placeholder : `${value.length}개 모델 선택됨`;
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            disabled={isLoading}
          >
            <span className="truncate text-left">{isLoading ? "모델을 불러오는 중..." : label}</span>
        ...
        <DropdownMenuContent align="start" className="w-[var(--radix-dropdown-menu-trigger-width)] max-h-64">
          ...
          {filtered.length === 0 ? (
            <div className="px-2 py-1.5 text-xs text-muted-foreground">모델을 찾을 수 없습니다</div>
          ) : null}
        </DropdownMenuContent>
      </DropdownMenu>
