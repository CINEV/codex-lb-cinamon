...
export function AccountListItem({
  ...
  const providerSubtitle = isPlatformIdentity
    ? `${formatProviderLabel(account.providerKind)} API key${account.routingSubjectId ? ` | Subject ${account.routingSubjectId}` : ""}`
  ...
  const showMonthlyRow = !isPlatformIdentity && monthlyOnly;
  const showPrimaryRow =
    !isPlatformIdentity && !monthlyOnly && hasPrimaryWindow && (quotaDisplay !== "weekly" || !hasSecondaryWindow);
  const showSecondaryRow =
    !isPlatformIdentity && !monthlyOnly && hasSecondaryWindow && (quotaDisplay !== "5h" || !hasPrimaryWindow);
  const visibleQuotaRows = Number(showPrimaryRow) + Number(showSecondaryRow) + Number(showMonthlyRow);
  const showRoutingPolicy = !isPlatformIdentity && status !== "reauth" && status !== "deactivated";
      ...
      <div className="flex items-center gap-2.5">
        <div className="min-w-0 flex-1">
          ...
          <p className="truncate text-xs text-muted-foreground" title={showAccountId ? `Account ID ${account.accountId}` : undefined}>
            {emailSubtitle ? <><span className={blurred ? "privacy-blur" : undefined}>{emailSubtitle}</span> | {providerSubtitle}{idSuffix}</> : <>{providerSubtitle}{idSuffix}</>}
          </p>
        </div>
        {showRoutingPolicy ? (
          <RoutingPolicyBadge
