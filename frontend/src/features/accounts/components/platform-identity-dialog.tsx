...
export function PlatformIdentityDialog({
  ...
  const [eligibleRouteFamilies, setEligibleRouteFamilies] = useState<PlatformRouteFamily[]>([]);
  ...
  useEffect(() => {
    ...
    if (isEdit && account) {
      ...
      setEligibleRouteFamilies(initialEligibleRouteFamilies);
      return;
    }
    resetForm();
  }, [account, initialEligibleRouteFamilies, initialLabel, initialOrganization, initialProject, isEdit, open]);
  ...
  const handleRouteToggle = (routeFamily: PlatformRouteFamily, checked: CheckedState) => {
    setEligibleRouteFamilies((current) => {
      ...
      return PLATFORM_ROUTE_FAMILY_ORDER.filter((value) => next.has(value));
    });
  };
